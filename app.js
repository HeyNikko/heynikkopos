...
const CUSTOM_CATEGORIES_KEY='heynikko_pos_custom_categories';
function getCustomCategories(){
  try{return JSON.parse(localStorage.getItem(CUSTOM_CATEGORIES_KEY)||'[]').filter(Boolean)}
  catch{return[]}
}
...
const CURRENCY_META={SGD:{symbol:'S$',digits:2,name:'Singapore Dollar'},TWD:{symbol:'NT$',digits:0,name:'New Taiwan Dollar'}};
function currencyMeta(code){return CURRENCY_META[code]||{symbol:`${code||'SGD'} `,digits:2,name:code||'Currency'}}
function eventCurrency(e){return e?.currencyCode||'SGD'}
function eventExchangeRate(e){const n=Number(e?.exchangeRate);return Number.isFinite(n)&&n>0?n:1}
function eventRounding(e){const n=Number(e?.priceRounding);return Number.isFinite(n)&&n>0?n:0}
function roundEventPrice(v,e){const step=eventRounding(e);return step?Math.round((Number(v)||0)/step)*step:(Number(v)||0)}
function eventBaseToLocal(v,e){return roundEventPrice((Number(v)||0)*eventExchangeRate(e),e)}
function eventUnitPrice(product,e){
  if(!product)return 0;
  const code=eventCurrency(e);

  // Direct market price takes priority.
  if(code==='TWD'){
    const twd=Number(product.twdPrice);
    if(Number.isFinite(twd)&&twd>0)return roundEventPrice(twd,e);
  }

  // SGD stays on the catalogue's base SGD price.
  if(code==='SGD')return roundEventPrice(Number(product.price)||0,e);

  // Fallback for unpriced foreign-market products.
  return roundEventPrice((Number(product.price)||0)*eventExchangeRate(e),e);
}
function productHasDirectEventPrice(product,e){
  if(!product)return false;
  if(eventCurrency(e)==='SGD')return true;
  if(eventCurrency(e)==='TWD')return Number(product.twdPrice)>0;
  return false;
}
function moneyCurrency(v,code){const m=currencyMeta(code),n=Number(v)||0;return `${m.symbol}${n.toLocaleString('en-SG',{minimumFractionDigits:m.digits,maximumFractionDigits:m.digits})}`}
function eventMoney(v,e){return moneyCurrency(v,eventCurrency(e))}
function saleMoney(v,s){return moneyCurrency(v,s?.currencyCode||'SGD')}
function salePricingContext(s){const e=eventById(s?.eventId);return{currencyCode:s?.currencyCode||eventCurrency(e),exchangeRate:Number(s?.exchangeRate)||eventExchangeRate(e),priceRounding:eventRounding(e)}}
const nowISO=()=>new Date().toISOString();const uid=p=>p+Date.now().toString(36)+Math.random().toString(36).slice(2,7);function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}function toast(msg){const t=$('#toast');t.textContent=msg;t.classList.add('show');setTimeout(()=>t.classList.remove('show'),2200)}function prod(id){return db.products.find(p=>p.id===id)}function eventById(id){return db.events.find(e=>e.id===id)}function currentEvent(){const e=eventById(db.currentEventId);return e&&e.status==='open'?e:null}function activeSale(s){return(s.status||'active')!=='voided'}function manualCart(){return db.cart.filter(i=>!i.promo)}
...
function renderCategoryOptions(){const cats=allCategories();const opts='<option value="">Select category…</option>'+cats.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('');$('#productCategory').innerHTML=opts;$('#categoryFilter').innerHTML='<option value="">All categories</option>'+cats.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('');$('#promoCategoryChecks').innerHTML=cats.map(c=>`<label class="category-check"><input type="checkbox" value="${esc(c)}"><span>${esc(c)}</span></label>`).join('');if(posCategoryState&&!cats.includes(posCategoryState))posCategoryState='';$('#categoryFilter').value=posCategoryState;renderPosCategoryButtons()}
function renderEventBanner(){const e=currentEvent();$('#eventBanner').innerHTML=e?`<div><strong>📍 ${esc(e.name)}</strong><span>${fmtDate(e.start)}${e.end&&e.end!==e.start?' – '+fmtDate(e.end):''} · ${eventCurrency(e)}</span></div><button class="ghost" data-goto-events>Manage Event</button>`:`<div><strong>No active event selected</strong><span>Create an event and allocate stock before taking booth sales.</span></div><button class="primary" data-create-event>+ Create Event</button>`;const ce=$('[data-create-event]');if(ce)ce.onclick=openCreateEvent;const ge=$('[data-goto-events]');if(ge)ge.onclick=()=>switchView('events')}
function renderProducts(){const q=($('#search')?.value||'').toLowerCase(),cat=posCategoryState||'',e=currentEvent();const cf=$('#categoryFilter');if(cf&&cf.value!==cat)cf.value=cat;const list=db.products.filter(p=>(!e||e.activeProducts?.[p.id]||(Number(e.stock?.[p.id])||0)>0)&&(!cat||p.category===cat)&&(p.name+' '+p.sku+' '+p.category).toLowerCase().includes(q));$('#productGrid').innerHTML=list.map(p=>{const stock=e?availableStock(p.id):0;return`<button class="product-card" data-add="${p.id}" ${!e||stock<=0?'disabled':''}>${productImageHtml(p,'product-card-image')}<div class="product-card-info"><strong>${esc(p.name)}</strong><div><span class="category-pill">${esc(p.category||'Uncategorised')}</span></div><div class="sku">${esc(p.sku)}</div><div class="price">${eventMoney(eventUnitPrice(p,e),e)}${eventCurrency(e)==='TWD'?`<div class="muted">${productHasDirectEventPrice(p,e)?'TWD direct price':'FX fallback'}</div>`:''}</div><div class="${stock<=p.low?'stock-low':'muted'}">Event stock: ${stock}</div></div></button>`}).join('')||'<p class="muted">No products found.</p>';$$('[data-add]').forEach(b=>b.onclick=()=>addToCart(b.dataset.add))}
function renderCart(){const e=currentEvent();$('#cart').innerHTML=db.cart.length?db.cart.map(r=>{const p=prod(r.productId);if(!p)return'';return`<div class="cart-row"><div class="cart-product">${productImageHtml(p,'cart-thumb')}<div><strong>${esc(p.name)}</strong> ${r.promo?'<span class="promo-pill">FREE PROMO</span>':''}<div class="muted">${esc(p.sku)} · ${r.promo?eventMoney(0,e):eventMoney(eventUnitPrice(p,e),e)}</div></div></div><div class="qty">${r.promo?`× ${r.qty}`:`<button class="ghost" data-minus="${r.id}">−</button><strong>${r.qty}</strong><button class="ghost" data-plus="${r.id}">+</button>`}</div></div>`}).join(''):'<p class="muted">Tap a product to start an order.</p>';const pricing=calcBundlePricing(manualCart(),e),lines=pricing.applied.filter(a=>a.discount>0).map(a=>`<div class="bundle-line"><span>${esc(a.label)} × ${a.bundles}</span><strong>−${eventMoney(a.discount,e)}</strong></div>`).join('');$('#bundleDiscounts').innerHTML=lines;$('#subtotalRow').style.display=pricing.discount>0?'flex':'none';$('#subtotal').textContent=eventMoney(pricing.subtotal,e);$('#total').textContent=eventMoney(pricing.total,e);$$('[data-minus]').forEach(b=>b.onclick=()=>changeQty(b.dataset.minus,-1));$$('[data-plus]').forEach(b=>b.onclick=()=>changeQty(b.dataset.plus,1))}
async function checkout(method){
  ...
  const pricing=calcBundlePricing(manualCart(),e),
    sale={
      id:uid('S'),
      ...
      status:'active',
      eventId:e.id,
      eventName:e.name,currencyCode:eventCurrency(e),exchangeRate:eventExchangeRate(e),
      items:[],
      cloudSynced:false
    };

  for(const r of db.cart){
    const p=prod(r.productId),unit=r.promo?0:eventUnitPrice(p,e);
    ...
    db.movements.push({id:uid('m'),createdAt:nowISO(),productId:p.id,sku:p.sku,name:p.name,delta:-r.qty,scope:'event',eventId:e.id,eventName:e.name,currencyCode:eventCurrency(e),exchangeRate:eventExchangeRate(e),reason:r.promo?'Promo gift':'Sale',receipt:sale.receipt});
  }
...
function renderProductTable(){
  ...
  $('#productsTable').innerHTML=db.products.map(p=>`<tr>
    ...
    <td><strong>${money(p.price)}</strong><div class="muted">${Number(p.twdPrice)>0?moneyCurrency(p.twdPrice,'TWD'):'TWD not set'}</div></td>
...
function bindProductDialogDismiss(){
  const d=$('#productDialog');
  if(!d||d.dataset.dismissBound==='1')return;
  d.dataset.dismissBound='1';
  ...
  const closeFromBackdrop=e=>{
    ...
    if(outside)closeProductDialog();
  };
  d.addEventListener('click',closeFromBackdrop);
  d.addEventListener('pointerup',closeFromBackdrop);
}
function openProduct(id=''){bindProductDialogDismiss();renderCategoryOptions();const p=id?prod(id):null;$('#productDialogTitle').textContent=p?'Edit Product':'Add Product';$('#productId').value=p?.id||'';$('#productSku').value=p?.sku||'';$('#productName').value=p?.name||'';$('#productCategory').value=p?.category||'';$('#productPrice').value=p?.price??'';$('#productTwdPrice').value=Number(p?.twdPrice)>0?p.twdPrice:'';$('#productStock').value=p?.stock??0;$('#productLow').value=p?.low??5;$('#productImage').value='';setImagePreview(p?.image||'');$('#productDialog').showModal()}
...
function bindStockAdjustmentDismiss(){
  const d=$('#stockDialog');
  if(!d||d.dataset.dismissBound==='1')return;
  d.dataset.dismissBound='1';
  ...
  const closeFromBackdrop=e=>{
    ...
    const r=d.getBoundingClientRect();
    const outside=e.clientX<r.left||e.clientX>r.right||e.clientY<r.top||e.clientY>r.bottom;
    if(outside)closeStockAdjustmentDialog();
  };

  d.addEventListener('click',closeFromBackdrop);
  d.addEventListener('pointerup',closeFromBackdrop);
}
function openStock(id){const p=prod(id);bindStockAdjustmentDismiss();$('#stockProductId').value=id;$('#stockProductName').textContent=`${p.name} · Master available ${p.stock}`;$('#stockDelta').value='';$('#stockReason').value='';const d=$('#stockDialog');if(!d.open)d.showModal()}
...
function closeEventStockDialog(){
  const d=$('#eventStockDialog');
  if(d?.open)d.close();
}
function bindEventStockDialogDismiss(){
  const d=$('#eventStockDialog');
  if(!d||d.dataset.dismissBound==='1')return;
  d.dataset.dismissBound='1';
  ...
  d.addEventListener('cancel',e=>{
    e.preventDefault();
    closeEventStockDialog();
  });

  // iPad / Safari backdrop tap:
  // a click whose coordinates land outside the dialog's content rectangle
  // is treated as a backdrop dismissal.
  ...
  // Some iPad Safari versions are more reliable with pointerup than click.
...
function bundlePromoAvailableForEvent(pr,e=currentEvent()){
  return bundlePriceForEvent(pr,e)!==null;
}
...
function promoCurrencies(pr,kind){
  if(kind==='gift')return['UNIVERSAL'];
  const out=[];
  if(Number(pr.bundlePrice)>0)out.push('SGD');
  if(Number(pr.twdBundlePrice)>0)out.push('TWD');
  return out;
}
function ensurePromoCurrencyFilter(){
  if(promoCurrencyFilter)return promoCurrencyFilter;
  const c=eventCurrency(currentEvent());
  promoCurrencyFilter=(c==='SGD'||c==='TWD')?c:'all';
  return promoCurrencyFilter;
}
function setPromoCurrencyFilter(v){
  promoCurrencyFilter=v||'all';
  localStorage.setItem('heynikko_promo_currency_filter',promoCurrencyFilter);
  renderPromos();
}
function promoMatchesCurrency(pr,kind,filter){
  if(filter==='all')return true;
  const currencies=promoCurrencies(pr,kind);
  return currencies.includes(filter)||currencies.includes('UNIVERSAL');
}
function promoCurrencyBadges(pr,kind){
  const currencies=promoCurrencies(pr,kind);
  if(!currencies.length)return'<span class="muted">—</span>';
  return `<div class="promo-currency-badges">${currencies.map(c=>`<span class="currency-pill ${c.toLowerCase()}">${c==='UNIVERSAL'?'Universal':c}</span>`).join('')}</div>`;
}
function promoOfferText(pr,kind,filter){
  if(kind==='gift'){
    const g=prod(pr.gift);
    return `${esc(g?.name||'Missing product')} × ${pr.giftQty} free`;
  }
  const bits=[];
  if((filter==='all'||filter==='SGD')&&Number(pr.bundlePrice)>0)bits.push(`<div>${pr.qty} for ${money(pr.bundlePrice)}</div>`);
  if((filter==='all'||filter==='TWD')&&Number(pr.twdBundlePrice)>0)bits.push(`<div>${pr.qty} for ${moneyCurrency(pr.twdBundlePrice,'TWD')}</div>`);
  return bits.join('')||'<span class="muted">—</span>';
}
function renderPromos(){
  const filter=ensurePromoCurrencyFilter();

  $$('.promo-currency-tab').forEach(btn=>btn.classList.toggle('active',btn.dataset.promoCurrency===filter));

  const bundleRows=db.bundlePromos
    .filter(pr=>promoMatchesCurrency(pr,'bundle',filter))
    .sort((a,b)=>bundleTargetLabel(a).localeCompare(bundleTargetLabel(b)))
    .map(pr=>`<tr>
      <td>Bundle price</td>
      <td>${esc(bundleTargetLabel(pr))}</td>
      <td>${promoCurrencyBadges(pr,'bundle')}</td>
      <td>${promoOfferText(pr,'bundle',filter)}</td>
      <td>${pr.active?'Active':'Disabled'}</td>
      <td><div class="action-row">
        <button class="ghost" data-edit-bundle="${pr.id}">Edit</button>
        <button class="ghost" data-toggle-bundle="${pr.id}">${pr.active?'Disable':'Enable'}</button>
        <button class="ghost" data-delete-bundle="${pr.id}">Delete</button>
      </div></td>
    </tr>`);

  const giftRows=db.promos
    .filter(pr=>promoMatchesCurrency(pr,'gift',filter))
    .map(pr=>{
      const b=prod(pr.buy);
      return`<tr>
        <td>Free gift</td>
        <td>${esc(b?.name||'Missing product')} × ${pr.buyQty}</td>
        <td>${promoCurrencyBadges(pr,'gift')}</td>
        <td>${promoOfferText(pr,'gift',filter)}</td>
        <td>${pr.active?'Active':'Disabled'}</td>
        <td><div class="action-row">
          <button class="ghost" data-edit-promo="${pr.id}">Edit</button>
          <button class="ghost" data-toggle-promo="${pr.id}">${pr.active?'Disable':'Enable'}</button>
          <button class="ghost" data-delete-promo="${pr.id}">Delete</button>
        </div></td>
      </tr>`;
    });

  const rows=[...bundleRows,...giftRows];
  $('#promosTable').innerHTML=rows.join('')||'<tr><td colspan="6" class="muted">No promotions for this currency.</td></tr>';

  const summary=$('#promoCurrencySummary');
  if(summary){
    const total=bundleRows.length+giftRows.length;
    summary.textContent=`${filter==='all'?'All currencies':filter} · ${total} promotion${total===1?'':'s'}`;
  }

  $$('[data-edit-bundle]').forEach(b=>b.onclick=()=>openEditPromo(b.dataset.editBundle,'bundle'));
  $$('[data-edit-promo]').forEach(b=>b.onclick=()=>openEditPromo(b.dataset.editPromo,'gift'));
  $$('[data-toggle-bundle]').forEach(b=>b.onclick=async()=>{const p=db.bundlePromos.find(x=>x.id===b.dataset.toggleBundle);p.active=!p.active;markPromoPending(p.id);persistLocal();renderAll();await syncPendingPromotions(false)});
  $$('[data-delete-bundle]').forEach(b=>b.onclick=()=>deletePromotion(b.dataset.deleteBundle,'bundle'));
  $$('[data-toggle-promo]').forEach(b=>b.onclick=async()=>{const p=db.promos.find(x=>x.id===b.dataset.togglePromo);p.active=!p.active;markPromoPending(p.id);persistLocal();recalcPromos();renderAll();await syncPendingPromotions(false)});
  $$('[data-delete-promo]').forEach(b=>b.onclick=()=>deletePromotion(b.dataset.deletePromo,'gift'));
}
...
function openPromo(){
  ...
  editingPromoKind='';
  promoOptions();
  $('#promoDialogTitle').textContent='Add Promotion';
  $('#promoSaveBtn').textContent='Save';

  $('#promoType').disabled=false;
  $('#promoType').value='bundle';
  $('#bundleTargetType').value='categories';
  togglePromoFields();

  $$('#promoCategoryChecks input').forEach(x=>x.checked=false);
  $('#promoProductSearch').value='';
  renderPromoProductChecks();
  $$('#promoProductChecks input').forEach(x=>x.checked=false);
  updatePromoProductCount();

  $('#bundleQty').value=5;
  $('#bundlePrice').value='';
  $('#bundleTwdPrice').value='';

  $('#promoDialog').showModal();
}
function openEditPromo(id,kind){
  promoOptions();

  editingPromoId=id;
  editingPromoKind=kind;

  $('#promoDialogTitle').textContent='Edit Promotion';
  $('#promoSaveBtn').textContent='Save Changes';
  $('#promoType').disabled=true;
  $('#promoType').value=kind==='bundle'?'bundle':'gift';
  togglePromoFields();

  if(kind==='bundle'){
    const pr=db.bundlePromos.find(x=>x.id===id);
    if(!pr)return toast('Promotion not found');

    $('#bundleTargetType').value=pr.targetType==='products'?'products':'categories';
    toggleBundleTargetFields();

    $$('#promoCategoryChecks input').forEach(x=>x.checked=(pr.categories||[]).includes(x.value));

    $('#promoProductSearch').value='';
    renderPromoProductChecks();
    $$('#promoProductChecks input').forEach(x=>x.checked=(pr.productIds||[]).includes(x.value));
    updatePromoProductCount();

    $('#bundleQty').value=pr.qty||1;
    $('#bundlePrice').value=Number(pr.bundlePrice)>0?pr.bundlePrice:'';
    $('#bundleTwdPrice').value=Number(pr.twdBundlePrice)>0?pr.twdBundlePrice:'';
  }else{
    const pr=db.promos.find(x=>x.id===id);
    if(!pr)return toast('Promotion not found');

    $('#promoBuy').value=pr.buy||'';
    $('#promoBuyQty').value=pr.buyQty||1;
    $('#promoGift').value=pr.gift||'';
    $('#promoGiftQty').value=pr.giftQty||1;
  }
...
async function savePromoForm(e){
  ...
  let promo,isEdit=!!editingPromoId;

  if($('#promoType').value==='bundle'){
    const targetType=$('#bundleTargetType').value,
      categories=$$('#promoCategoryChecks input:checked').map(x=>x.value),
      productIds=$$('#promoProductChecks input:checked').map(x=>x.value),
      qty=+$('#bundleQty').value,
      bundlePrice=Math.max(0,+$('#bundlePrice').value||0),
      twdBundlePrice=Math.max(0,+$('#bundleTwdPrice').value||0);
    ...
    if(isEdit){
      promo=db.bundlePromos.find(x=>x.id===editingPromoId);
      if(!promo)return toast('Promotion not found');
      promo.targetType=targetType;
      promo.categories=targetType==='categories'?categories:[];
      promo.productIds=targetType==='products'?productIds:[];
      promo.qty=qty;
      promo.bundlePrice=bundlePrice;
      promo.twdBundlePrice=twdBundlePrice;
    }else{
      promo={
        id:uid('bp'),type:'bundle',targetType,
        categories:targetType==='categories'?categories:[],
        productIds:targetType==='products'?productIds:[],
        qty,bundlePrice,twdBundlePrice,active:true
      };
      db.bundlePromos.push(promo);
    }
  }else{
    const buy=$('#promoBuy').value,
      buyQty=+$('#promoBuyQty').value,
      gift=$('#promoGift').value,
      giftQty=+$('#promoGiftQty').value;

    if(!buy||!gift)return toast('Choose both Buy and Gift products');
    if(!buyQty||buyQty<1||!giftQty||giftQty<1)return toast('Promo quantities must be at least 1');

    if(isEdit){
      promo=db.promos.find(x=>x.id===editingPromoId);
      if(!promo)return toast('Promotion not found');
      promo.buy=buy;
      promo.buyQty=buyQty;
      promo.gift=gift;
      promo.giftQty=giftQty;
    }else{
      promo={id:uid('pr'),type:'gift',buy,buyQty,gift,giftQty,active:true};
      db.promos.push(promo);
    }
  }

  markPromoPending(promo.id);
  persistLocal();
  ...
  if(cloudSession&&sb&&navigator.onLine){
    toast(isEdit?'Promotion updated · syncing…':'Promotion saved · syncing…');
    const ok=await syncPendingPromotions(false);
    if(ok){
      await pullCloudPromotions();
      renderAll();
      toast(isEdit?'Promotion updated · cloud synced':'Promotion saved · cloud synced');
    }else{
      toast(isEdit?'Promotion updated locally · cloud sync pending':'Promotion saved locally · cloud sync pending');
    }
  }else{
    toast(isEdit?'Promotion updated offline · queued for sync':'Promotion saved offline · queued for sync');
  }
}
...
function salesRangeBounds(){
  ...
  if(salesDateFrom){
    from=new Date(`${salesDateFrom}T00:00:00`);
  }
  if(salesDateTo){
    to=new Date(`${salesDateTo}T23:59:59.999`);
  }
  return{from,to};
}
function saleInSelectedDateRange(s){
  const d=new Date(s.createdAt),{from,to}=salesRangeBounds();
  if(Number.isNaN(d.getTime()))return false;
  if(from&&d<from)return false;
  if(to&&d>to)return false;
  return true;
}
function salesRangeDescription(){
  if(!salesDateFrom&&!salesDateTo)return'All Time';
  const fmt=v=>v?new Date(`${v}T00:00:00`).toLocaleDateString('en-SG',{day:'numeric',month:'short',year:'numeric'}):'Beginning';
  return `${fmt(salesDateFrom)} – ${salesDateTo?fmt(salesDateTo):'Today'}`;
}
function setSalesDateRange(from,to){
  salesDateFrom=from||'';
  salesDateTo=to||'';
  localStorage.setItem('heynikko_sales_date_from',salesDateFrom);
  localStorage.setItem('heynikko_sales_date_to',salesDateTo);
  const f=$('#salesDateFrom'),t=$('#salesDateTo');
  if(f)f.value=salesDateFrom;if(t)t.value=salesDateTo;
  renderSales();
}
function applySalesRangePreset(type){
  const now=new Date();
  if(type==='all')return setSalesDateRange('','');
  if(type==='today'){
    const v=localDateInputValue(now);return setSalesDateRange(v,v);
  }
  if(type==='7days'){
    const from=new Date(now);from.setDate(from.getDate()-6);
    return setSalesDateRange(localDateInputValue(from),localDateInputValue(now));
  }
  if(type==='month'){
...
function updateSalesDateFilterUI(){
  const f=$('#salesDateFrom'),t=$('#salesDateTo'),label=$('#salesDateRangeLabel');
  if(f&&f.value!==salesDateFrom)f.value=salesDateFrom;
  if(t&&t.value!==salesDateTo)t.value=salesDateTo;
  if(label)label.textContent=salesRangeDescription();

  $$('.sales-range-preset').forEach(b=>b.classList.remove('active'));
  const now=new Date(),today=localDateInputValue(now);
  let active='';
  if(!salesDateFrom&&!salesDateTo)active='all';
  else if(salesDateFrom===today&&salesDateTo===today)active='today';
  else{
    const seven=new Date(now);seven.setDate(seven.getDate()-6);
    const month=new Date(now.getFullYear(),now.getMonth(),1);
    if(salesDateFrom===localDateInputValue(seven)&&salesDateTo===today)active='7days';
    if(salesDateFrom===localDateInputValue(month)&&salesDateTo===today)active='month';
  }
  if(active){
    const b=$(`[data-sales-range="${active}"]`);if(b)b.classList.add('active');
  }
}
function renderSales(){
  updateSalesDateFilterUI();

  const allVisible=db.sales.filter(s=>!s.deletedPending),
    visibleSales=allVisible.filter(saleInSelectedDateRange),
    active=visibleSales.filter(activeSale),
    sumByCurrency=a=>{const m=new Map();for(const s of a){const c=s.currencyCode||'SGD';m.set(c,(m.get(c)||0)+(Number(s.total)||0))}return m},
    units=a=>a.reduce((n,s)=>n+(s.items||[]).reduce((x,i)=>x+(Number(i.qty)||0),0),0);

  const rangeLabel=salesRangeDescription();

  $('#salesSummary').innerHTML=
    `<div class="stat"><span>${esc(rangeLabel)} · Revenue</span><strong>${[...sumByCurrency(active)].map(([c,v])=>moneyCurrency(v,c)).join(' + ')||moneyCurrency(0,'SGD')}</strong></div>`+
    `<div class="stat"><span>Transactions</span><strong>${active.length}</strong></div>`+
    `<div class="stat"><span>Units Sold</span><strong>${units(active)}</strong></div>`;

  $('#salesTable').innerHTML=visibleSales.map(s=>{
    const e=eventById(s.eventId),
      locked=e?.status==='closed',
      checked=selectedSaleIds.has(s.id)?'checked':'';

    return`<tr class="${activeSale(s)?'':'voided-row'}">
      <td class="select-col"><input class="sales-check" type="checkbox" data-sale-select="${s.id}" ${checked} aria-label="Select ${esc(s.receipt)}"></td>
      <td>${new Date(s.createdAt).toLocaleString('en-SG')}</td>
      <td>${esc(s.eventName||'Legacy / Master')}</td>
      <td>${esc(s.payment)}</td>
      <td>${s.items.reduce((a,i)=>a+i.qty,0)}</td>
      <td>${saleMoney(s.total,s)}</td>
      <td><span class="status-pill ${activeSale(s)?'active':'voided'}">${activeSale(s)?(s.editedAt?'EDITED':'COMPLETED'):'VOIDED'}${activeSale(s)&&!s.cloudSynced?' · SYNC PENDING':''}${!activeSale(s)&&!s.cloudVoidSynced?' · VOID PENDING':''}</span></td>
      <td><div class="action-row">
        <button class="ghost" data-view-sale="${s.id}">View</button>
        ${activeSale(s)&&!locked?`<button class="ghost" data-edit-sale="${s.id}">Edit</button>`:''}
        ${activeSale(s)?`<button class="danger-btn" data-void-sale="${s.id}">Void</button>`:''}
        <button class="danger-btn" data-delete-sale="${s.id}">Delete</button>
      </div></td>
    </tr>`;
  }).join('')||'<tr><td colspan="8" class="muted">No sales in the selected date range.</td></tr>';

  $$('[data-sale-select]').forEach(c=>c.onchange=()=>{
    if(c.checked)selectedSaleIds.add(c.dataset.saleSelect);
    else selectedSaleIds.delete(c.dataset.saleSelect);
    updateSalesBulkControls();
  });
  $$('[data-view-sale]').forEach(b=>b.onclick=()=>viewSale(b.dataset.viewSale));
  $$('[data-edit-sale]').forEach(b=>b.onclick=()=>openEditSale(b.dataset.editSale));
  $$('[data-void-sale]').forEach(b=>b.onclick=()=>voidSale(b.dataset.voidSale));
  $$('[data-delete-sale]').forEach(b=>b.onclick=()=>deleteSalePermanently(b.dataset.deleteSale));
  updateSalesBulkControls();
}
...
function viewSale(id){const s=db.sales.find(x=>x.id===id);if(!s)return;$('#saleViewContent').innerHTML=`<div class="receipt-meta"><div><span>Receipt</span><strong>${esc(s.receipt)}</strong></div><div><span>Event</span><strong>${esc(s.eventName||'Legacy / Master')}</strong></div><div><span>Date</span><strong>${new Date(s.createdAt).toLocaleString('en-SG')}</strong></div><div><span>Payment</span><strong>${esc(s.payment)}</strong></div><div><span>Currency</span><strong>${esc(s.currencyCode||'SGD')}</strong></div><div><span>Status</span><strong>${activeSale(s)?(s.editedAt?'Edited':'Completed'):'Voided'}</strong></div><div><span>Total</span><strong>${saleMoney(s.total,s)}</strong></div></div><div class="receipt-lines">${s.items.map(i=>`<div class="receipt-line"><span>${esc(i.name)} ${i.promo?'<span class="promo-pill">FREE</span>':''}<small>${esc(i.sku)} · × ${i.qty}</small></span><strong>${i.promo?'FREE':saleMoney(i.unitPrice*i.qty,s)}</strong></div>`).join('')}</div>${s.bundleDiscount?`<div class="receipt-line"><span>Bundle discount</span><strong>−${saleMoney(s.bundleDiscount,s)}</strong></div>`:''}`;$('#saleViewDialog').showModal()}
...
function renderEditSale(){const s=db.sales.find(x=>x.id===$('#editSaleId').value),allow=editAllowance(s),gifts=calcPromoItems(editSaleDraft,allow),ctx=salePricingContext(s),pricing=calcBundlePricing(editSaleDraft,ctx);$('#editSaleItems').innerHTML=[...editSaleDraft.map((r,idx)=>{const p=prod(r.productId);return`<div class="edit-line"><button type="button" class="ghost remove-mini" data-edit-remove="${idx}">Remove</button><div>${esc(p?.name||'Missing')}<div class="muted">${esc(p?.sku||'')}</div></div><div class="qty"><button type="button" class="ghost" data-edit-minus="${idx}">−</button><strong>${r.qty}</strong><button type="button" class="ghost" data-edit-plus="${idx}">+</button></div></div>`}),...gifts.map(g=>{const p=prod(g.productId);return`<div class="edit-line promo-edit"><span class="promo-pill">FREE</span><div>${esc(p?.name||'Missing')}</div><strong>× ${g.qty}</strong></div>`})].join('');$('#editSaleTotal').textContent=eventMoney(pricing.total,ctx);$$('[data-edit-remove]').forEach(b=>b.onclick=()=>{editSaleDraft.splice(+b.dataset.editRemove,1);renderEditSale()});$$('[data-edit-minus]').forEach(b=>b.onclick=()=>{const r=editSaleDraft[+b.dataset.editMinus];r.qty--;if(r.qty<=0)editSaleDraft.splice(+b.dataset.editMinus,1);renderEditSale()});$$('[data-edit-plus]').forEach(b=>b.onclick=()=>{const r=editSaleDraft[+b.dataset.editPlus];if(r.qty+1>allow[r.productId])return toast('Insufficient stock');r.qty++;renderEditSale()})}
...
function cloudRowToLocalProduct(r,old=null){
  return{id:old?.id||`cloud_${r.id}`,cloudId:r.id,sku:r.sku,name:r.name,category:r.category||'',price:Number(r.price)||0,twdPrice:Number(r.twd_price)||0,stock:Number(r.master_qty)||0,low:Number(r.low_stock_at)||0,image:r.image_url||'',active:r.active!==false};
}
...
function cloudFingerprint(p){return JSON.stringify([p.sku||'',p.name||'',p.category||'',Number(p.price)||0,Number(p.twdPrice)||0,Number(p.stock)||0,Number(p.low)||0,cloudImageKey(p),p.active!==false])}
...
function eventFingerprint(e){
  return JSON.stringify({
    name:e.name||'',start:e.start||'',end:e.end||'',status:e.status||'open',closedAt:e.closedAt||'',currencyCode:eventCurrency(e),exchangeRate:eventExchangeRate(e),priceRounding:eventRounding(e),
...
async function pushSaleToCloud(sale){
  ...
  if(sale.cloudId&&sale.editedAt){
    const result=await sb.rpc('update_pos_sale_details',{
      ...
      p_edited_at:sale.editedAt,p_currency_code:sale.currencyCode||'SGD',p_exchange_rate:Number(sale.exchangeRate)||1,
      p_items:items
    });
    ...
    const result=await sb.rpc('record_pos_sale',{
      ...
      p_discount:Number(sale.bundleDiscount||0),
      p_total:Number(sale.total)||0,
      p_created_at:sale.createdAt||nowISO(),p_currency_code:sale.currencyCode||'SGD',p_exchange_rate:Number(sale.exchangeRate)||1,
      p_items:items
    });
    if(result.error)throw result.error;
    data=result.data;
  ...
  try{
    ...
    let rebuilt=sales.filter(r=>!pendingDeletedEventCloudIds().has(String(r.event_id))).map(r=>{
      ...
      return{
        ...
        currencyCode:r.currency_code||old?.currencyCode||eventCurrency(ev),exchangeRate:Number(r.exchange_rate)||old?.exchangeRate||eventExchangeRate(ev),
        items:saleItems
      };
    });
...
async function upsertEventToCloud(ev){
  const eventPayload={
    ...
    currency_code:eventCurrency(ev),exchange_rate:eventExchangeRate(ev),price_rounding:eventRounding(ev),
    updated_at:nowISO()
  };
  ...
  try{
    ...
    for(const r of events){
      ...
      const ev={
        ...
        currencyCode:r.currency_code||old?.currencyCode||'SGD',exchangeRate:Number(r.exchange_rate)||old?.exchangeRate||1,priceRounding:Number(r.price_rounding)||old?.priceRounding||0,
...
async function fetchCloudProducts(){if(!cloudSession||!sb)throw new Error('Sign in first');const {data,error}=await sb.from('products').select('id,sku,name,category,price,image_url,master_qty,low_stock_at,active,updated_at,twd_price').order('name');if(error)throw error;return data||[]}
...
function renderPosCategoryButtons(){
  ...
  const cats=allCategories(),available=new Set(cats);
  if(posCategoryState&&!available.has(posCategoryState))posCategoryState='';
  const current=posCategoryState;
  wrap.innerHTML=['',...cats].map(c=>`<button type="button" class="v72-category-btn ${c===current?'active':''}" data-pos-category="${esc(c)}">${c||'All'}</button>`).join('');
